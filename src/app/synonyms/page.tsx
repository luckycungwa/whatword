      ...
      {wordsWithSynonyms.length > 0 ? (
        <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {wordsWithSynonyms.map(word => (
            <Link key={word.slug} href={`/synonyms/${word.slug}`} className="group rounded-2xl border border-[#e0e0e0] bg-white p-5 transition-all hover:border-[#0066ff]/30">
        ...
        <div className="mt-12 text-center text-[#707070]">
          <p className="text-lg">No synonyms cached yet</p>
          <p className="mt-1 text-sm">Search for a word to see its synonyms</p>
        </div>
      )}
    </div>
  );
