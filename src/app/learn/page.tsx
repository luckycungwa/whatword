...
import { ArrowRight, Library, BookOpen, GraduationCap, Trophy } from 'lucide-react';
...
export const metadata: Metadata = {
  ...
  openGraph: {
    title: 'Vocabulary Builder — Learn English Words by Level',
    description: 'Build your English vocabulary with structured learning paths organised by difficulty.',
    url: 'https://whatword.co.za/learn',
  },
};
      ...
      <div className="mt-10">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {levels.map(({ level, label, description, color }) => {
            ...
            return (
              <Link key={level} href={`/learn/${level.toLowerCase()}`} className="group rounded-2xl border border-[#e0e0e0] bg-white p-5 transition-all hover:border-[#e0e0e0]">
                <div className="flex items-start justify-between">
                  <div>
                    <div className="flex items-center gap-2">
                      <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${
                        color === 'green' ? 'bg-[#f3f3f3] text-[#707070]' :
                        color === 'yellow' ? 'bg-[#e0e0e0] text-[#141414]' :
                        'bg-[#e0e0e0] text-[#141414]'
                      }`}>{level}</span>
                      <span className="font-semibold text-[#141414] group-hover:text-[#141414]">{label}</span>
                    </div>
      ...
      <div className="mt-16">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          {(['beginner', 'intermediate', 'advanced'] as const).map(diff => {
            ...
            return (
              <Link key={diff} href={`/learn/${diff}`} className="group rounded-2xl border border-[#e0e0e0] bg-white p-5 transition-all hover:border-[#e0e0e0]">
                <div className="flex items-start justify-between">
                  <div>
                    <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-bold ${
                      diff === 'beginner' ? 'bg-[#f3f3f3] text-[#707070]' :
                      diff === 'intermediate' ? 'bg-[#e0e0e0] text-[#141414]' :
                      'bg-[#e0e0e0] text-[#141414]'
                    }`}>{diff.charAt(0).toUpperCase() + diff.slice(1)}</span>
      ...
      <div className="mt-16">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
          {categories.map(cat => {
            ...
            return (
              <Link key={cat} href={`/learn/${cat.toLowerCase().replace(/\s+/g, '-')}`} className="rounded-2xl border border-[#e0e0e0] bg-white p-4 text-center transition-all hover:border-[#e0e0e0]">
