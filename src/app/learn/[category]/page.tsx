...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: `Learn ${title} English vocabulary.`,
...
export default async function LearnCategoryPage({ params }: Props) {
  ...
  if (isLevel) {
    ...
    words = allWords.filter(w => w.vocabularyLevel === level);
    title = `Level ${level} Vocabulary`;
  } else if (isDifficulty) {
    words = allWords.filter(w => w.difficulty === category);
    ...
    <div className="container-app py-8 md:py-12">
      ...
      {words.length > 0 ? (
        <div className="mt-8 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {words.map(word => (
            <Link key={word.slug} href={`/words/${word.slug}`} className="group flex items-start justify-between rounded-2xl border border-[#e0e0e0] bg-white p-4 transition-all hover:border-[#0066ff]/30">
        ...
        <div className="mt-8 rounded-2xl border border-[#e0e0e0] bg-white p-8 text-center">
          ...
          <Link href="/learn" className="mt-3 inline-block text-sm font-medium text-[#0066ff] hover:underline">Browse all levels →</Link>
        </div>
      )}
    </div>
  );
